...
class photon:
    def __init__(self, window, mass, position = np.array([-2e9, 0], dtype = float), direction = np.array([1,0], dtype = float)):
        ...
        f = 1 - mass.r_s / self.r
        self.dt = np.sqrt((self.dr*self.dr) / (f*f) + (self.r*self.r * self.dtheta*self.dtheta) / f)
        self.E = f * self.dt       #FIXED
    ...
    def check_boundary(self):
        ...
        if d2 < r_s:
            ...
            return
        if self.position[0] > right or self.position[0] < left or self.position[1] < top or self.position[1] > bottom:
            self.out_of_bounds = True
    ...
    def bend(self):
        ...
        d2r = -r_s * f * self.dt * self.dt / (2 * r * r) + r_s * self.dr * self.dr / (2 * r * r * f) + (r - r_s) * self.dtheta * self.dtheta
